...
public class TournamentTeam
{
	...
	public void backInstance()
	{
		for (Player player : getMembers())
		{
			// Remove player from tournament instance
			if (player.isInTournamentInstance())
			{
				int instanceId = player.getTournamentInstanceId();
				net.sf.l2j.gameserver.model.entity.instance.Instance instance = InstanceManager.getInstance().getInstance(instanceId);
				if (instance != null)
				{
				...
				// Clean known list to remove instance-specific objects and refresh world objects
				net.sf.l2j.commons.pool.ThreadPool.schedule(() -> 
				{
					player.cleanInstanceKnownList();
					// Refresh to see world NPCs again
					player.refreshInfos();
				}, 500);
			}
		}
