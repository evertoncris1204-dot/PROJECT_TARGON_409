...
public class TournamentFight
{
	...
	public TournamentFight(int id, TournamentFightType fightType, Instance instance, TournamentTeam teamOne, TournamentTeam teamTwo, TournamentArena tournamentArena)
	{
		...
		// Validate that we have a valid instance - each fight MUST have its own unique instance
		if (instance == null)
		{
			TournamentManager.getInstance().debugInfo("[Tournament Fight]: ERROR - Fight " + id + " created without an instance!");
			throw new IllegalStateException("TournamentFight cannot be created without an instance!");
		}
		
		// Ensure this instance is unique and not shared with other fights
		if (TournamentManager.getInstance().getCurrentFights().values().stream()
			.anyMatch(f -> f != null && f.getInstance() != null && f.getInstance().getId() == instance.getId()))
		{
			TournamentManager.getInstance().debugInfo("[Tournament Fight]: ERROR - Instance " + instance.getId() + " is already in use by another fight!");
			throw new IllegalStateException("Instance " + instance.getId() + " is already assigned to another fight!");
		}
		
		// Set instance ID for all team members - ensure they are assigned to THIS instance only
		for (Player player : teamOne.getMembers())
		{
			if (player.isInTournamentInstance())
			{
				TournamentManager.getInstance().debugInfo("[Tournament Fight]: WARNING - Player " + player.getName() + " is already in instance " + player.getTournamentInstanceId() + "! Resetting to new instance " + instance.getId());
			}
			player.setTournamentInstanceId(instance.getId());
		}
		...
		{
			TournamentManager.getInstance().debugInfo("[Tournament Fight]: Fight " + id + " assigned to unique instance " + instance.getId() + " with " + (teamOne.getMembers().size() + teamTwo.getMembers().size()) + " players");
		}
	...
	public void finish()
	{
		...
		if (getWinner() != null)
		{
			...
			// Announce victory to all online players
			String fightTypeName = getFightTypeName();
			String winnerMembers = getTeamMembersString(getWinner());
			String loserMembers = getLooser() != null ? getTeamMembersString(getLooser()) : "Unknown";
			World.announceToOnlinePlayers("[Tournament] " + fightTypeName + ": " + winnerMembers + " defeated " + loserMembers + "!", false);
		...
		else
		{
			...
			// Announce tie to all online players
			String fightTypeName = getFightTypeName();
			String teamOneMembers = getTeamMembersString(teamOne);
			String teamTwoMembers = getTeamMembersString(teamTwo);
			World.announceToOnlinePlayers("[Tournament] " + fightTypeName + ": " + teamOneMembers + " vs " + teamTwoMembers + " ended in a tie!", false);
		}
			...
			// Doors remain closed - don't open them when fight ends
			
			// Remove players from instance and delete instance - ensure complete cleanup
			if (instance != null)
			{
				...
				for (Player player : teamOne.getMembers())
				{
					if (player.getTournamentInstanceId() == instanceId)
					{
						instance.removePlayer(player);
						player.setTournamentInstanceId(-1);
					}
				}
				for (Player player : teamTwo.getMembers())
				{
				...
				// Delete instance - this ensures it cannot be reused by another fight
				net.sf.l2j.gameserver.model.entity.instance.InstanceManager.getInstance().deleteInstance(instanceId);
				
				if (Config.TOURNAMENT_DEBUG)
				{
					TournamentManager.getInstance().debugInfo("[Tournament Fight]: Fight " + id + " finished - deleted instance " + instanceId);
				}
	...
	public void finish(TournamentTeam winner)
	{
		...
		// Announce victory to all online players
		String fightTypeName = getFightTypeName();
		String winnerMembers = getTeamMembersString(winner);
		String loserMembers = looser != null ? getTeamMembersString(looser) : "Unknown";
		World.announceToOnlinePlayers("[Tournament] " + fightTypeName + ": " + winnerMembers + " defeated " + loserMembers + "!", false);
	...
	/**
	 * Get a formatted string representation of the fight type
	 */
	private String getFightTypeName()
	{
		switch (fightType)
		{
			case F1X1:
	...
	/**
	 * Get a formatted string with team members names
	 */
	private String getTeamMembersString(TournamentTeam team)
	{
		...
		{
			Player member = team.getMembers().get(i);
