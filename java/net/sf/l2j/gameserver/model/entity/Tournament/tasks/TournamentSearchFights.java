	...
	public void run()
	{
		...
		if (teams.size() >= 2)
		{
			try
			{
				...
				List<TournamentTeam> searchList = new ArrayList<>(teams);
				...
				if (found)
				{
					...
					// Create a UNIQUE instance for this match - each fight gets its own isolated instance
					Instance instance = InstanceManager.getInstance().createInstance();
					
					if (instance == null)
					{
						TournamentManager.getInstance().debugInfo("[Tournament Search Fights]: Failed to create instance for fight!");
						return;
					}
					...
					if (arena != null)
					{
						...
						if (Config.TOURNAMENT_DEBUG)
						{
							TournamentManager.getInstance().debugInfo("A new fight (ID: " + fight.getId() + ", Instance ID: " + instance.getId() + ") [" + fightType.name() + "] started: " + teamOne.getName() + " vs " + teamTwo.getName());
						}
						
					}
					else
					{
						// If arena is null, delete the instance we created
						InstanceManager.getInstance().deleteInstance(instance.getId());
						TournamentManager.getInstance().debugInfo("[Tournament Search Fights]: Arena is null, cannot create fight!");
					}
